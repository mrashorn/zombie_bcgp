...
int main()
{
	...
	std::ifstream inputFile("gamedata/scores.txt"); // pass file name to ifstream constructor
	...
	while (window.isOpen())
	{
		...
		*/
					...
					if (bullets[i].isInFlight() && zombies[j].isAlive())
					{
						if (bullets[i].getPosition().intersects(zombies[j].getPosition()))
						{
							...
							if (zombies[j].hit())
							{
								...
								// When all of the zombies are dead (again)
								if (numZombiesAlive == 0)
								{
									state = State::LEVELING_UP;
								}
							}
							// make the splat sound when bullet hits zombie
							splat.play();
						}
					}
				}
		...
		for (int i = 0; i < numZombies; i++)
		{
			if (player.getPosition().intersects(zombies[i].getPosition()) && zombies[i].isAlive())
			{
				...
				if (player.getHealth() <= 0)
				{
					...
					std::ofstream outputFile("gamedata/scores.txt"); // pass the file name to the constructor of ofstream
					// << writes the data
					outputFile << hiScore;
					outputFile.close();
		...
		if (framesSinceLastHUDUpdate > fpsMeasurementFrameInterval)
		{
			...
			// Update the number of zombies text
